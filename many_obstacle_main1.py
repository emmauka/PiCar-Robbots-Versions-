...
def main():
    ...
    # Now perform the maneuvers
    while True:
        if move_forward(durations["right1"]):
            fc.turn_right(speed)
            time.sleep(turn_duration)
            fc.stop()

        if move_forward(durations["left1"]):
            fc.turn_left(speed)
            time.sleep(turn_duration)
            fc.stop()

        if move_forward(durations["forward1"]):
            fc.turn_left(speed)
            time.sleep(turn_duration)
            fc.stop()

        if move_forward(durations["left2"]):
            fc.turn_left(speed)
            time.sleep(turn_duration)
            fc.stop()

        if move_forward(durations["right2"]):
            fc.turn_right(speed)
            time.sleep(turn_duration)
            fc.stop()

        if move_forward(durations["left3"]):
            fc.turn_left(speed)
            time.sleep(turn_duration)
            fc.stop()
            
        # After completing the path, stop the car
        fc.stop()
        break
